...
const f8 = () => {
    let res = a8.reduce(
        (accum, item) => (item > 0 ? (accum += item) : accum),
        0
    );
...
const f9 = () => {
    document.querySelector('.out-9').textContent = a9.reduce(
        (accum, item) => (item > 0 ? (accum *= item) : accum),
        1
    );
};
...
const f12 = () => {
    let res = a12.reduce(
        (accum, item, index, arr) => (item > arr[accum] ? index : accum),
        0
    );
...
const f13 = () => {
    let res = a13.reduce((accum, item) =>
        accum.length > item.length ? accum : item
    ).length;
...
let a15 = [
    0, 0, 1, 1, 1, 0, 2, 2, 3, 3, 3, 4, 5, 5, 6, 4, 4, 3, 1, 1, 0, 0, -1,
];
...
const f16 = () => {
    ...
    document.querySelector('.out-16').innerHTML = Object.entries(res)
        .map((item) => item.join(': '))
        .join('; ');
};
...
const f20 = () => {
    let res = a20.reduce((accum, item, i, arr) => {
        accum.unshift(item);
