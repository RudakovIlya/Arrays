...
let a4 = [3, -2, 4, 1, '9', -3, '0', true, 2, -8, undefined, NaN];
...
const f6 = () => {
    z6 = a6.filter(
        (item) => item.reduce((accum, item) => (accum += item)) % 2 === 0
    );
...
const f10 = () => {
    for (const key in a10) {
        if (callback10(a10[key])) z10[key] = a10[key];
    }
